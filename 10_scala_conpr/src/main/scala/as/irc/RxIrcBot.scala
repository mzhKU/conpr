...
class RxIrcBot(out: OutputStreamWriter, in: ConnectableObservable[String]) {
    ...
    def init(): Unit = {
        ...
        in.subscribe(msg => if (isPing(msg)) { sendPong("PONG") })
        
        // Handle messages
        in.filter(msg => msg.startsWith(":rot13"))
          .subscribe(rot13Msg => sendMsg(rot13(rot13Msg)))
        
        in.subscribe(msg => if(isPrivMsg(msg)) { sendMsg("Hallo " + senderNick(msg)) } )
